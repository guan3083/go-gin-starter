...
// ws 的所有连接
// 用于广播
var WsConnAll map[string]*wsConnection = make(map[string]*wsConnection)
...
func WsHandler(c *gin.Context) {
	...
	token := c.Query("token")
	claims, code := CheckTokenString(token)
	if code != e.SUCCESS {
	...
	log.Println("当前在线人数", len(WsConnAll), "当前在线用户:", userId)
...
func (wsConn *wsConnection) close() {
	...
	if wsConn.isClosed == false {
		...
		// 删除这个连接的变量
		delete(WsConnAll, "10000")
		close(wsConn.closeChan)
	}
}

func Send(name, data string) {
	WsConnAll[name].outChan <- &wsMessage{1, []byte(data)}
}
